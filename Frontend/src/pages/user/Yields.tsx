...
export default function Yields() {
  ...
  const API_URL = "http://localhost:5000/api/yields";
  ...
  const addYield = async () => {
    ...
    try {
      const res = await fetch(API_URL, {
        ...
        body: JSON.stringify({
          ...newYield,
          yield: parseFloat(newYield.yield)
        }),
      });
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-end">
              <Button onClick={addYield} className="w-full bg-gradient-farm hover:opacity-90 shadow-farm">
